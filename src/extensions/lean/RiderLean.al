...
% for the Whipple bicycle model with a leaning rider and four inputs.
%---------------------------------------------------------------------%
%         default settings
%---------------------------------------------------------------------%
...
% declare the four points
% dn: rear contact point
% fn: front contact point
% ce: steer axis point
% cg: upper body hinge point
...
% d4: the perpendicular distance from the rear wheel center to the
%     hinge line
...
%     the center of mass of the fork
% l5: the distance in the g1> direction from the upper body hinge to
%     the center of mass of the upper body
% l6: the distance in the g3> direction from the upper body hinge to
%     the center of mass of the upper body

constants rf, rr, lam, d{1:4}, l{1:6}
...
inertia g, ig11, ig22, ig33, 0, 0, ig31
...
% rear wheel center to hinge location
p_do_cg> = -d4 * sin(lam) * c1> + d4 * cos(lam) * c3>

% hinge location to upper body mass center
p_cg_go> = l5 * g1> + l6 * g3>
...
% rate, u7, and the upper body lean rate, u9, are taken to be the
...
input q1=0.0 m, q2=0.0 m, q3=0.0 rad, q4=0.0 rad, q5=0.0 rad, &
      q6=0.0 rad, q7=0.0 rad, q8=0.0 rad, q9=0.0 rad
...
output u1 m/s, u2 m/s, u3 rad/s, u4 rad/s, u5 rad/s, u6 rad/s, &
       u7 rad/s, u8 rad/s, u9 rad/s

code dynamics() RiderLeanDynamics.c
